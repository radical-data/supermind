...
function isAdmin(request: Request, kind: AuthKind): boolean {
	...
	if (kind === "basic" || kind === "any") {
		if (parsed.scheme === "basic" && validateBasic(parsed.value, adminToken))
			return true;
	}

	if (kind === "bearer" || kind === "any") {
		if (parsed.scheme === "bearer" && validateBearer(parsed.value, adminToken))
			return true;
	}
...
export function adminBasicResponseIfUnauthorised(
	request: Request,
): Response | null {
